...
def dashboard_data():
    ...
    total = sum(amount(r["Total"].get("UnblendedCost", {})) for r in daily.get("ResultsByTime", []))
    previous_total = sum(amount(r["Total"].get("UnblendedCost", {})) for r in previous.get("ResultsByTime", []))
    service_rows = []
    if services.get("ResultsByTime"):
        for group in services["ResultsByTime"][0].get("Groups", []):
            ...
            service_rows.append({"service": keys[0], "cost": value})
    service_rows.sort(key=lambda x: x["cost"], reverse=True)

    forecast = safe(
        lambda: client("ce", CE_REGION).get_cost_forecast(
    ...
    for b in budget_data.get("Budgets", []):
        ...
        budgets.append({"name": b.get("BudgetName"), "limit": limit, "type": b.get("BudgetType")})
    ...
    return {
        ...
        "daily": [
            {"date": r.get("TimePeriod", {}).get("Start"), "cost": amount(r.get("Total", {}).get("UnblendedCost", {}))}
            for r in daily.get("ResultsByTime", [])
        ],
...
def resources_data():
    ...
    for reservation in ec2.get("Reservations", []):
        for i in reservation.get("Instances", []):
            name = next((t.get("Value") for t in i.get("Tags", []) if t.get("Key") == "Name"), i.get("InstanceId"))
            instances.append({"name": name, "id": i.get("InstanceId"), "type": "EC2", "region": REGION, "status": i.get("State", {}).get("Name")})
...
def monitoring_data():
    ...
    def metric(namespace, name, dimensions):
        result = safe(lambda: cw.get_metric_statistics(
            Namespace=namespace,
            MetricName=name,
    ...
    ec2_avg = metric("AWS/EC2", "CPUUtilization", [])
    rds_avg = metric("AWS/RDS", "CPUUtilization", [])
    return {"ec2_cpu": ec2_avg, "rds_cpu": rds_avg, "updated_at": end.isoformat() + "Z"}
...
def security_data():
    ...
    return {
        ...
        "iam_users": iam_summary.get("SummaryMap", {}).get("Users", 0),
        "iam_roles": iam_summary.get("SummaryMap", {}).get("Roles", 0),
        "waf_acls": len(waf.get("WebACLs", [])),
    }
...
def budgets_data():
    ...
    return [{
        ...
        "forecast": float(b.get("CalculatedSpend", {}).get("ForecastedSpend", {}).get("Amount", 0) or 0),
...
def handler(event, context):
    ...
    routes = {
        ...
        "/api/cost": dashboard_data,
        "/api/resources": resources_data,
        "/api/monitoring": monitoring_data,
        "/api/security": security_data,
        "/api/budgets": budgets_data,
    }
    if path in routes:
        try:
